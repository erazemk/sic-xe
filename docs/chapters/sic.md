...
The `A` register is used when doing integer arithmetic operations, where its value is modified (added to, subtracted from,  etc.) with the value from the address, specified in the instruction.
...
The `PC` register is used for storing the next instruction's address, so that the microprocessor know where to find the instruction to execute.
The address stored in the register is implicitly (automatically) incremented after executing each instruction.
The value of the `PC` register doesn't normally need to be set manually, so SIC doesn't offer a separate instruction for it.

### The `SW` register

The `SW` register is split into multiple meaningful sections of bits, whicheach hold useful information:
- `Bit 0`: Mode
...
It can find the memory address of that value (the **target address** or TA) from the address bits, but by combining the addressing bits with the indexing bit value, it can do this in two ways:
